...
```  
这段代码主要先通过queryLocalInterface方法来查询远端server是否和当前client在同一进程，其中obj.queryLocalInterface(DESCRIPTOR)调用的IBinder接口，其实现是在Binder类中，代码入下：
```java
//from Binder.java
...
```
如果在同一进程则返回本地的service,既Stub。此时不需要跨进程，直接使用本地的service。如果不是同一进程则通过Proxy来获得远端service的引用，这时asInterface()的参数iBinder为BinderProxy对象。BinderProxy是Binder的内部类和Binder一样实现了IBinder接口，BinderProxy是Server端Binder的代理对象,可以通过proxy里调用远端server的服务。本例中的Service是依附在Android中的Service组件上，可以在在Manifest中通过Process属性来定义service是运行在独立进程还是和client同一进程。
```java
//from manifest.xml
...
```
调用service的方法后，主要是通过mRemote.tansact()方法来跨进程和server端通信。前面提到过mRemote会根据client和server是否在同一进程而不同，如果在同一进程则其为Binder对象既本地的Stub的实现类IFabonacciService对象。如果不在同一进程则其为BinderProxy对象，既此时调用BinderProxy的transact方法。
```java
